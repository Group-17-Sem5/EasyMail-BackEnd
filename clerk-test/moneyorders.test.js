...
test("GET /api/clerk/moneyorder", async () => {
  await User.deleteMany({})
await Postman.deleteMany({})
await Branch.deleteMany({})
  const branch = await Branch.create({
    branchName:"Bopay",
    mobileNumber:'0918655434',
    address:"Galle"
  })
  ...
  const moneyOrder = await MoneyOrder.create({ 
  ...
  await supertest(app).get("/api/clerk/post/")
    .expect(200)
    .then((response) => {
